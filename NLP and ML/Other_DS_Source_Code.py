...
data_set = pd.read_csv("input/other_dataset.csv")
...
def main():
    BILLBOARD_DATASET = {'Len Avg': 143, 'Most Rep Avg': 15.78, 'Avg Rep': 9.21, 'Unique Avg': 68.5,
                         'Normalize Length': 183}
    ...
    for artist_name, songs in group_data:
        ...
        for index, rows in songs.iterrows():
            if rows["song"] not in all_songs_in_set:
                ...
                artists_all_songs_list.append(
                    {song_name: [song_length, most_rep_num, avg_rep_num, unique_words_len, list(clean_text_list)]})
    ...
    dataset_overview = {"Total Songs in DataSet": dataset_total_num_of_songs, "Total Words": length_of_dataset,
                        "Unique Words": unique_words_length_of_dataset,
                        "Avg Length Per Song": avg_len_of_song_in_dataset}

    print(f"Overview of Complete DataSet Values  : {dataset_overview}")
    print(
        f"Transformed Dataset extracted Features [ Len Avg, Most Rep Avg, Avg Rep, Unique Avg, Normalize Length] :{dataset_features}")
    ...
    for song in all_songs_in_set:
        ...
        final_data.append(
            round((weight0 * 0.25 + weight1 * 0.12 + weight2 * 0.12 + weight3 * 0.15 + weight4 * 0.18 + weight5 * 0.18),
                  2))
    ...
    df.to_csv('input/otherds_abs_features_dataset.csv')
    ...
    dfn.to_csv('input/otherds_norm_features_dataset.csv')
